module AresMUSH
  module Txt
    ...
    def self.format_telegram_indicator(char, names)
      t('telegram.telegram_indicator',
     :start_marker => Global.read_config("telegram", "telegram_start_marker") || "(", :end_marker => Global.read_config("telegram", "telegram_end_marker") || ")",  :preface => Global.read_config("telegram", "telegram_preface"),  :recipients => names, :color => Txt.telegram_color(char) )
    end

    def self.format_recipient_display_names(recipients, sender)
      use_nick = Global.read_config("telegram", "use_nick")
      ...
      return t('telegram.recipient_indicator', :recipients => recipients)
    end
    ...
    def self.format_recipient_names(recipients)
      ...
      return t('telegram.recipient_indicator', :recipients => recipient_names.join(" "))
    end

    def self.telegram_color(char)
      char.telegram_color || "%xh%xy"
    end

    def self.telegram_recipient(sender, recipient, recipient_names, message, scene_id = nil)
